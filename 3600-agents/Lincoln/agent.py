...
class PlayerAgent:
    """
    Lincoln v3 - Nixon +++++:
    - Full HMM belief tracking (transition matrix + sensor update)
    - High-value carpet snap (>= 10 pts instant take)
    - Opponent carpet pattern detection with blocking primes
    - Search risk budget (tracks failed searches, tightens thresholds)
    - Expected value gate for searches
    - Single unified scoring function for all move types
    """
    ...
    def play(self, board: board.Board, sensor_data, time_left: Callable = None):
        ...
        if best_search is not None and self._should_search(board, best_search[1]):
            return best_search[0]

        if not non_search:
            return random.choice(moves)

        best = max(non_search, key=lambda m: self._score(board, m))
        self._prev_opp = board.opponent_worker.get_location()
        return best
    ...
    def _score(self, board, m):
        ...
        if mt == enums.MoveType.CARPET:
            ...
            s = 2.80 * pts + 0.10 * fc + 0.04 * adj + 0.30 * rb - cp
            if pts <= 0:
                s -= 1.8
            elif pts == 2:
                s -= 0.25
        ...
        if mt == enums.MoveType.PRIME:
            s = 1.65 + 0.70 * fc + 0.22 * adj + 0.24 * rb - cp
            if tl <= 10:
                s += 0.25
            return s + block_bonus

        if mt == enums.MoveType.PLAIN:
            s = -0.80 + 0.75 * fc + 0.15 * adj + 0.55 * rb - cp
            if tl <= 8:
                s += 0.20
            return s + block_bonus
    ...
    def _should_search(self, board, p):
        tl = self._turns_remaining(board)
        top3 = sum(sorted(self.belief, reverse=True)[:3])
        ...
        # Tighten thresholds after repeated failures
        penalty = min(self._failed_searches * 0.04, 0.16)

        if p >= 0.58 - penalty:
            return True
        if tl <= 10 and p >= 0.48 - penalty:
            return True
        if tl <= 6 and p >= 0.42 - penalty:
            return True
        if top3 >= 0.75 and p >= 0.50 - penalty:
            return True
        if top5 >= 0.88 and p >= 0.46 - penalty:
            return True
        return False
