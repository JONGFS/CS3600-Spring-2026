...
class PlayerAgent:
    def __init__(
        ...
        self.opp_search_count = 0
        self.last_opp_turns_left: int | None = None
    ...
    def _synchronize_belief(self, board: game_board.Board, sensor_data: Tuple):
        first_call = board.turn_count == 0 and self.last_turn_seen == -1
        if first_call:
            self.belief = list(self.search_prior)

        if (
            board.opponent_search != self.last_opponent_search
            and board.opponent_search[0] is not None
        ):
            self.opp_search_count += 1
        self.last_opp_turns_left = board.opponent_worker.turns_left

        # Belief evolves in lockstep with the engine's rat ticks. The engine
        # calls rat.move() at the top of every player's turn, so between two of
        # OUR consecutive observations the rat has moved twice (opp's turn +
        # ours). Likewise, player_search was made when the rat was at iter N,
        # opponent_search when it was at iter N+1, and the current sample is at
        # iter N+2 — so feedback and advances must interleave in that order.
        if first_call:
            predicted = self.belief
        else:
            self._apply_search_feedback(board.player_search)
            advanced_once = self._advance_belief_once(self.belief)
            self.belief = advanced_once
    ...
    def _candidate_search_moves(
        ...
        my_now_pts = self._best_immediate_carpet_points(board, enemy=False)

        # Minimum belief required to even consider a search. Empirically our old
        # floor (p ≥ 0.31) produced ~35% hit rate; opp winners sit at ~60%+.
        min_p = 0.50
        if point_margin < -3 and turns_left > 8:
            min_p = 0.55  # behind — don't gamble
        if turns_left <= 6:
            min_p = 0.40  # desperate endgame, loosen

        # Adaptive: against a search-spam opponent (e.g. AdamSmasher at ~9/40
        # turns ≈ 22%), our conservative floor leaves too much EV on the table.
        # Lower the bar when they're searching aggressively.
        opp_turns_taken = max(1, 40 - board.opponent_worker.turns_left)
        opp_search_rate = self.opp_search_count / opp_turns_taken
        if opp_search_rate >= 0.20 and point_margin >= -3:
            min_p = max(0.40, min_p - 0.08)
        ...
        for idx in top:
            ...
            # Never burn a turn searching when we have a guaranteed ≥4 pt carpet.
            if my_now_pts >= 4.0 and ev < 2.0:
                continue
    ...
    # ------------------------------------------------------------------
    # Negamax search with alpha-beta + TT
    # ------------------------------------------------------------------
    ...
    def _evaluate(self, board: game_board.Board, belief: Sequence[float]) -> float:
        ...
        score += 8.0 * my_now - 13.5 * opp_now       # slightly higher opp penalty
        score += 4.0 * my_next - 6.8 * opp_next
        score += 3.5 * my_access - 5.5 * opp_access   # NEW: accessibility
        score += 1.8 * (my_mob - opp_mob)
        score += block_term
        score += 2.5 * search_term                    # slightly higher search weight

        if opp_now >= 6.0:
            score -= 42.0                             # stronger threat penalty
        if opp_now >= 10.0:
            score -= 60.0
        ...
        if turns_left <= 12:
            ...
            score += 8.0 * my_now
            score -= 20.0 * opp_now                  # more aggressive late-game defense
            score -= 8.0 * opp_next
            if point_margin > 0:
    ...
    def _move_heuristic(
        ...
        if mv.move_type == enums.MoveType.PRIME:
            return (
                270.0
                + 40.0 * self._future_carpet_points_from(board, dest)  # up from 34
        ...
        return (
            ...
            + 6.0 * self._future_carpet_points_from(board, dest)  # NEW
        )
