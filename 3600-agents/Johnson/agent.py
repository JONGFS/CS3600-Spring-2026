...
TT_LOWER = 1
TT_UPPER = 2
MOVE_CATEGORIES = ("carpet", "block", "prime", "mobility", "search", "plain")
...
class PlayerAgent:
    def __init__(
        ...
        self.category_regret = {cat: 0.0 for cat in MOVE_CATEGORIES}
        self.category_strategy_sum = {cat: 1e-6 for cat in MOVE_CATEGORIES}
        self.last_root_category_values = {cat: 0.0 for cat in MOVE_CATEGORIES}
    ...
    def play(
        ...
        for depth in range(1, max_depth + 1):
            if time_left() <= max(0.02, deadline):
                break
            try:
                value, chosen, category_values = self._search_root(
                    board,
                    self.belief,
                    ordered_moves,
                    depth,
                ...
                break
            last_category_values = category_values
        ...
        if completed_depth == 0:
            best_move = ordered_moves[0]
            self._accumulate_category_strategy(board, ordered_moves)
        elif last_category_values is not None:
            self._accumulate_category_strategy(board, ordered_moves)
            self._update_category_regret(board, best_move, last_category_values)
    ...
    def _ordered_root_moves(
        ...
        category_strategy = self._available_category_strategy(board, self.belief, moves)
        scored = []
        for mv in moves:
            category = self._move_category(board, self.belief, mv)
            cfr_bonus = 90.0 * category_strategy.get(category, 0.0)
            scored.append((self._move_heuristic(board, mv, self.belief) + cfr_bonus, mv))
    ...
    def _search_root(
        ...
        deadline: float,
        time_left: Callable,
    ) -> Tuple[float, move.Move | None, Dict[str, float]]:
        alpha = -inf
        beta = inf
        best_move = None
        best_value = -inf
        best_biased_value = -inf
        category_values: Dict[str, float] = {}
        category_strategy = self._available_category_strategy(board, belief, ordered_moves)
        for mv in ordered_moves:
            ...
            category = self._move_category(board, belief, mv)
            category_values[category] = max(category_values.get(category, -inf), value)
            biased_value = value + 24.0 * category_strategy.get(category, 0.0)
            if biased_value > best_biased_value:
                best_biased_value = biased_value
                best_value = value
                best_move = mv
            if value > alpha:
                alpha = value
        return best_value, best_move, category_values
    ...
    def _current_category_strategy(self) -> Dict[str, float]:
        positive_regret = {cat: max(0.0, value) for cat, value in self.category_regret.items()}
        total = sum(positive_regret.values())
        if total <= 1e-9:
            uniform = 1.0 / len(MOVE_CATEGORIES)
            return {cat: uniform for cat in MOVE_CATEGORIES}
        return {cat: positive_regret[cat] / total for cat in MOVE_CATEGORIES}

    def _available_category_strategy(
        self,
        board: game_board.Board,
        belief: Sequence[float],
        ...
        if not available:
            return {}
        strategy = self._current_category_strategy()
        total = sum(strategy.get(category, 0.0) for category in available)
        if total <= 1e-9:
            uniform = 1.0 / len(available)
            return {category: uniform for category in available}
        return {category: strategy.get(category, 0.0) / total for category in available}

    def _accumulate_category_strategy(
        self,
        board: game_board.Board,
        moves: Sequence[move.Move],
    ) -> None:
        strategy = self._available_category_strategy(board, self.belief, moves)
        for category in MOVE_CATEGORIES:
            self.category_strategy_sum[category] *= 0.997
            self.category_strategy_sum[category] += strategy.get(category, 0.0)

    def _update_category_regret(
        self,
        board: game_board.Board,
        chosen_move: move.Move,
        category_values: Dict[str, float],
    ) -> None:
        chosen_category = self._move_category(board, self.belief, chosen_move)
        chosen_value = category_values.get(
            chosen_category, self._move_heuristic(board, chosen_move, self.belief)
        )
        for category in MOVE_CATEGORIES:
            self.category_regret[category] *= 0.985
        for category, value in category_values.items():
            delta = max(-45.0, min(45.0, value - chosen_value))
            self.category_regret[category] += delta
            self.last_root_category_values[category] = value
        self.last_root_category_values[chosen_category] = chosen_value
